...
function mostrarEspectaculos(infoEspectaculos){
    ...
    tiempoButton.textContent = "Proximos";
...
function ordenarShows(shows, modo){
    ...
    switch(modo){
        case 0: // Muestra los shows por areas
            shows.forEach(show => {
                // Iterar sobre cada clave dentro del show
                Object.keys(show).forEach(category => {
                    // Si la categoría tiene elementos
                    if (show[category].length > 0) {
                        // Mostrar el nombre y las horas de cada elemento en la categoría
                        show[category].forEach(item => {
                            texto += `\nArea: ${cambiarIDaNombreArea(category)}\nNombre: ${item.name}\nHoras: ${item.hours.length === 0 ? "Todo el dia" : item.hours.join(", ")}\n`;
                        });
                    }
                });
            });
